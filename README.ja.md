...
> **0.8.10 の新機能** — `docmd doctor` 事前チェック、`docmd init`/`docmd stop` ライフサイクル、`migrate --dry-run`/`--upgrade`、`docmd-search` によるセマンティック検索、OKF バンドル、`NO_COLOR` および `DOCMD_NO_BANNER` 環境変数。完全なリストは [リリースノート](https://github.com/docmd-io/docmd/releases/tag/v0.8.10) を参照してください。
...
docmd migrate --upgrade  # レガシーな docmd.config を最新スキーマに変換
docmd migrate --dry-run  # 何も書き込まずに移行をプレビュー
